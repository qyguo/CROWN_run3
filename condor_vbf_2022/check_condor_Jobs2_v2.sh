...
output_Path=/eos/user/j/jiahua/MC_2022_inc/MC_2022_VBF
...
resubOr=resub2
DataName=*
config=_run3_vbf_uncertainty_2022
JobID_condor=*
JobID_condor=13203983
...
do
    ...
    ##elif [[ ! $checkmain ]]; then
        ...
        if [[ ${DataOrMC} == true ]]; then
            run_script_tmp=./vbfhmm_config_run3_vbf_uncertainty_data_2022
        else
            run_script_tmp=./vbfhmm_config_run3_vbf_uncertainty_top_2022
        fi
        ...
        if [[ (! $checkmain) && -z $input_File ]]; then
            ...
            output_path=$(grep "/eos/user/j/jiahua/MC_2022_inc/" ${file})
            echo $output_path
