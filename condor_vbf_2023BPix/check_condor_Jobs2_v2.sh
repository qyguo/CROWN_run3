...
resubOr=resub1
DataName=*
config=_run3_vbf_uncertainty_2023BPix
JobID_condor=*
JobID_condor=130477
...
do
    ...
    ##elif [[ ! $checkmain ]]; then
        ...
        if [[ ${DataOrMC} == true ]]; then
            run_script_tmp=./vbfhmm_config_run3_vbf_HornVeto_data_2023BPix
        else
            run_script_tmp=./vbfhmm_config_run3_vbf_HornVeto_top_2023BPix
        fi
        ...
        if [[ (! $checkmain) && -z $input_File ]]; then
            ...
            output_path=$(grep "/eos/user/j/jiahua/MC_2023BPix_inc/" ${file})
            echo $output_path
