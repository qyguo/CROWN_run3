...
resubOr=resub1
DataName=*
config=_run3_Inc_v4_JetVeto_top_2023
JobID_condor=*
JobID_condor=11280046
...
do
    ...
    ##elif [[ ! $checkmain ]]; then
        ...
        echo "    running localy"
        if [[ ${DataOrMC} ]]; then
            run_script_tmp=./vbfhmm_config_run3_Inc_v4_JetVeto_data_2023
        else
            run_script_tmp=./vbfhmm_config_run3_Inc_v4_JetVeto_top_2023
        fi
        ...
        if [[ (! $checkmain) && -z $input_File ]]; then
            ...
            output_path=$(grep "/eos/user/j/jiahua/MC_2023_inc/" ${file})
            echo $output_path
